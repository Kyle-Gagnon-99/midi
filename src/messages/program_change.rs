...
const MIDI_EVENT_TYPE: u8 = 0xC0;

const PROGRAM_CHANGE_SIZE: u8 = 0x02;
...
impl Event for ProgramChangeEvent {
    ...
    fn to_bytes(&self) -> Result<Vec<u8>, MidiError> {
        let bytes: Vec<u8> = vec![(MIDI_EVENT_TYPE | self.channel), self.program_change];
    ...
    fn to_bytes_delta_time(&self) -> Result<Vec<u8>, MidiError> {
        ...
        bytes.extend_from_slice(&[(MIDI_EVENT_TYPE | self.channel), self.program_change]);
...
impl FromBytes for ProgramChangeEvent {
    type Output = Self;
    ...
    fn from_bytes(
        ...
        let program_change = data[1];

        Ok(Self {
            program_change,
            channel,
            event_size: PROGRAM_CHANGE_SIZE,
            delta_time,
