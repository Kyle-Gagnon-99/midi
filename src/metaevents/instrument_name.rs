...
const METAEVENT_BYTE_TYPE: u8 = 0x04;
...
impl Event for InstrumentNameEvent {
    fn get_event_name(&self) -> String {
        String::from("Instrument Name")
    }
    ...
    fn to_bytes(&self) -> Result<Vec<u8>, MidiError> {
        ...
        bytes.extend_from_slice(self.instrument_name.as_bytes());
...
impl FromBytes for InstrumentNameEvent {
    type Output = Self;
    ...
    fn from_bytes(
        ...
        let instrument_name = get_utf8_from_bytes(&data)?;

        // The 0xFF 0x01 bytes plus the number of bytes from the VLQ size + the size of the text data
        let event_size = 2 + num_of_bytes + instrument_name.len() as u8;
...
mod text_event_tests {
    use crate::{
        events::{Event, FromBytes},
        metadata::TimeDivision,
        metaevents::{from_vlq_to_bytes, instrument_name::METAEVENT_BYTE_TYPE, METAEVENT_BYTE},
    };

    use super::InstrumentNameEvent;

    #[test]
    fn create_instrument_name_struct_from_bytes_success_one_byte_vlq() {
        ...
        let result =
            InstrumentNameEvent::from_bytes(&bytes, 0, TimeDivision::PulsesPerQuarterNote(96), 120);
        assert!(result.is_ok());

        let text_event = result.unwrap();
        let event_size = 2 + test_text_len.len() as u8 + test_text.len() as u8;
        assert_eq!(text_event.instrument_name, test_text);
        assert_eq!(text_event.text_size, test_text.len() as u32);
        assert_eq!(text_event.get_event_size(), event_size);
    }

    #[test]
    fn create_instrument_name_struct_from_bytes_success_two_bytes_vlq() {
