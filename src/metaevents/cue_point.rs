...
const METAEVENT_BYTE_TYPE: u8 = 0x07;

#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "json", derive(Serialize))]
pub struct CuePointEvent {
    pub cue_point: String,
    text_size: u32,
    event_size: u8,
    delta_time: u32,
...
impl Event for CuePointEvent {
    fn get_event_name(&self) -> String {
        String::from("Cue Point")
    }
    ...
    fn to_bytes(&self) -> Result<Vec<u8>, MidiError> {
        ...
        bytes.extend_from_slice(self.cue_point.as_bytes());
...
impl FromBytes for CuePointEvent {
    type Output = Self;
...
mod text_event_tests {
    use crate::{
        events::{Event, FromBytes},
        metadata::TimeDivision,
        metaevents::{cue_point::METAEVENT_BYTE_TYPE, from_vlq_to_bytes, METAEVENT_BYTE},
    };

    use super::CuePointEvent;

    #[test]
    fn create_marker_struct_from_bytes_success_one_byte_vlq() {
        let test_text = String::from("Curtain Opens");
        ...
        let result =
            CuePointEvent::from_bytes(&bytes, 0, TimeDivision::PulsesPerQuarterNote(96), 120);
        assert!(result.is_ok());

        let text_event = result.unwrap();
        let event_size = 2 + test_text_len.len() as u8 + test_text.len() as u8;
        assert_eq!(text_event.cue_point, test_text);
