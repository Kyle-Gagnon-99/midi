...
const METAEVENT_BYTE_TYPE: u8 = 0x05;
...
impl Event for LyricEvent {
    ...
    fn to_bytes(&self) -> Result<Vec<u8>, MidiError> {
        ...
        bytes.extend_from_slice(self.lyric.as_bytes());
...
mod text_event_tests {
    use crate::{
        events::{Event, FromBytes},
        metadata::TimeDivision,
        metaevents::{from_vlq_to_bytes, lyric::METAEVENT_BYTE_TYPE, METAEVENT_BYTE},
    };

    use super::LyricEvent;

    #[test]
    fn create_lyric_struct_from_bytes_success_one_byte_vlq() {
        let test_text = String::from("When you need your great, Your great destroyer"); // Show some Coheed and Cambria love
